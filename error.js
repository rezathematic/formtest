...
function looper(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    let newArr = arr[i];
    (async () => {
      const url = "http://localhost:10053/form/"; // get URL
      ...
      await page.type("#acf-_post_title", newArr.title); // Type in the post Title
      ...
      for (let j = 0; j < newArr.asin.length; j++) {
        console.log(j + 1);
        let productTitle = await frameInputContent.type(
          `${j + 1}. [amazon fields="${newArr.asin[j]}" value="title"]`
        );
        await frameInputContent.type(String.fromCharCode(13));

        let productButton = await frameInputContent.type(
          `[amazon fields="${newArr.asin[j]}" value="button"]`
        );
        await frameInputContent.type(String.fromCharCode(13));

        let productDescription = await frameInputContent.type(
